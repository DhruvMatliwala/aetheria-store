...
function DiscordIcon({ className = "h-3 w-3" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      ...
      {filteredLeads.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredLeads.map((lead) => {
            ...
            } else if (lead.source === 'discord') {
              sourceBadgeClass = 'bg-[#121636] text-[#818cf8] border-[#5865F2]/60';
              sourceIcon = <DiscordIcon className="h-3 w-3 text-[#818cf8]" />;
            }
